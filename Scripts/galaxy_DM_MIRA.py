...
def Star_state((x,y,z,vx,vy,vz),t,dt,states):#(x1,y1,z1),(x2,y2,z2)): #state of star, t, positions of galactic cores
	...
	accns = -G * Mass_r(soft_dist_MW,r_s_M,r_200_M) * (r_star - r_MW)/soft_dist_MW**3 - G * Mass_r(soft_dist_Androm,r_s_A,r_200_A) * (r_star - r_A)/soft_dist_Androm**3
...
def Initalalise_stars_disk((x,y,z),(vx,vy,vz),N,radius,inc,rot,mass,gal): #N is no. of stars, radius is of galaxy, inc is angle to x, rot is rotation around z
	...
	#if gal == "MW": star_r[:,0] = Mass_r(star_r[:,0]*r_200_M,r_s_M,r_200_M)/Mass_r(r_200_M,r_s_M,r_200_M) * radius
	#elif gal == "A": star_r[:,0] = Mass_r(star_r[:,0]*r_200_A,r_s_A,r_200_A)/Mass_r(r_200_A,r_s_A,r_200_A) * radius
...
if __name__ == '__main__':
	...
	N_points = 1e3
	t_final = 5e9 #years
	print "Time step: %.1f years" % (t_final/N_points)
	...
	initial_cons = (-300e3 * pc,0,0,60e3,-5e3,-5e3,300e3 * pc,0,0,-60e3,15e3,5e3) #MW POS-VEL, ANDROM POS-VEL
	...
	'''data_reduction = 0#int(1e2)
	
	star_solns_A = star_solns_A[:,::data_reduction,:]
	star_solns_M = star_solns_M[:,::data_reduction,:]
	galaxy_states = galaxy_states[::data_reduction,:]
	time_base = time_base[::data_reduction]'''
	...
	np.save("TIME_BASE_DATA",time_base)
